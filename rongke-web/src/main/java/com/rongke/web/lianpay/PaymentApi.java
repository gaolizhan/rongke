...
public class PaymentApi {
	 ...
	 * @throws Exception
	 */
	public  static String payment(String orderNo ,String money,String bankNum,String bankName,String userName,String note,Long orderId) throws Exception {
		...
		paymentRequestBean.setDt_order(time);
		paymentRequestBean.setMoney_order(money);
		paymentRequestBean.setCard_no(bankNum);
		paymentRequestBean.setAcct_name(userName);
		paymentRequestBean.setBank_name(bankName);
		paymentRequestBean.setInfo_order(note);
		paymentRequestBean.setFlag_card("0");
		// 填写商户自己的接收付款结果回调异步通知 长度64位
		paymentRequestBean.setNotify_url("http://118.31.42.175/api/lianpay/tradepayapi/receiveNotify.htm?orderId="+orderId+"");
		...
		if (StringUtils.isEmpty(response)) {
			...
			if(result.equals("success")){
				...
				return result+","+paymentRequestBean.getNo_order();
			}
		} else {
